    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 800,
          system: "Eres un experto en nutrición vegana. Cuando el usuario escriba un alimento no vegano (carne, pescado, lácteos, huevos, etc.), propón 3-5 alternativas veganas concretas que lo sustituyan en cocina, indicando para qué uso es mejor cada una y sus macros aproximados por 100g. Sé práctico y usa productos fáciles de encontrar en España. Responde en español, de forma clara y concisa.",
          messages: [{ role: "user", content: `¿Qué alternativas veganas existen para sustituir: ${altQuery}?` }],
        }),
      });
      const data = await res.json();
      if (data.error) { setAltResponse("Error: " + data.error.message); setAltLoading(false); return; }
      const text = (data.content || []).filter(b => b.type === "text").map(b => b.text).join("\n");
      setAltResponse(text || "Sin respuesta.");
    } catch (e) {
      setAltResponse("Error de conexión: " + e.message);
    }
    setAltLoading(false);
  };
      ...
      <div style={ST.app}>
        ...
        <div style={{ padding: "20px 22px" }}>
          ...
          {tab === "alternativas" && (
            <>
              <div style={ST.cardGreen}>
                <div style={{ fontSize: 13, color: "#7fff7f", fontWeight: 600, marginBottom: 6 }}>¿Qué sustituye a...?</div>
                <div style={{ fontSize: 12, color: "#666", lineHeight: 1.7 }}>Escribe cualquier alimento no vegano y la IA te propone las mejores alternativas veganas con sus usos y macros.</div>
              </div>

              <div style={ST.card}>
                <div style={{ ...ST.label, fontSize: 10, marginBottom: 8 }}>Alimento a sustituir</div>
                <input
                  style={{ ...ST.input, marginBottom: 14 }}
                  placeholder="Ej: pollo, atún, queso, huevo, leche..."
                  value={altQuery}
                  onChange={e => setAltQuery(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && buscarAlternativas()}
                />
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                  {["Pollo", "Atún", "Carne picada", "Queso", "Leche", "Huevo", "Jamón", "Bacalao"].map(s => (
                    <button key={s} onClick={() => { setAltQuery(s); setAltResponse(""); }}
                      style={{ ...ST.btnOff, padding: "5px 10px", fontSize: 11 }}>
                      {s}
                    </button>
                  ))}
                </div>
                <button style={{ ...ST.btnPrimary, width: "100%", opacity: altLoading ? 0.6 : 1 }}
                  onClick={buscarAlternativas} disabled={altLoading}>
                  {altLoading ? "Buscando alternativas..." : "🔄 Buscar alternativas veganas"}
                </button>
              </div>

              {altResponse && (
                <div style={ST.card}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#7fff7f", marginBottom: 12 }}>
                    Alternativas para: <span style={{ color: "#e8e8e8" }}>{altQuery}</span>
                  </div>
                  <div style={{ fontSize: 13, color: "#bbb", lineHeight: 1.8, whiteSpace: "pre-wrap" }}>
                    {altResponse}
                  </div>
                </div>
              )}
